...
def scaling_s(x, sigma=0.1, plot=False):
    ...
    if plot:
        plot1d(x, x_, save_file='aug_examples/scal.png')
...
def rotation_s(x, plot=False):
    ...
    if plot:
        plot1d(x, x_, save_file='aug_examples/rotation_s.png')
    return x_
...
def magnitude_warp_s(x, sigma=0.2, knot=4, plot=False):
    ...
    if plot:
        plot1d(x, x_, save_file='aug_examples/magnitude_warp_s.png')
    return x_
...
def time_warp_s(x, sigma=0.2, knot=4, plot=False):
    ...
    for dim in range(x.shape[1]):
        ...
        ret[:, dim] = np.interp(orig_steps, np.clip(scale * time_warp, 0, x.shape[0] - 1),
                                x[:, dim]).T
    if plot:
       plot1d(x, ret, save_file='aug_examples/time_warp_s.png')
    return ret
...
def scaling_transform_vectorized(X, sigma=0.1):
    ...
    """
    scaling_factor = np.random.normal(loc=1.0, scale=sigma, size=(X.shape[0],X.shape[2]))
